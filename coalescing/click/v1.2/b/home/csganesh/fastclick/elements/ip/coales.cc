...
int Coales::initialize (ErrorHandler *) {
    /* From Padding */
...
int
Coales::configure(Vector<String> &conf, ErrorHandler *errh) // TODO consolidate functionality
{
...
void Coales::process_packet (Packet *p, bool start, bool end) {
...
Packet *
Coales::obfuscate(Packet *p, uint32_t obfuscate_offset)
{
  if (!_obfus) return p;
...
Packet *Coales::padding(Packet *p) {
    if (!_pad) return p;
...
void Coales::run_timer(Timer* ) {
    DEBUG_CHATTER ("In CL::Runtask last batch=%p",last_batch);
     _lock->acquire();
     if (!last_batch || last_batch->count() == 0) {
        if (_interval > 0) {
            _timer.schedule_after(_stamp);
            DEBUG_CHATTER ("timeout configured delta %d us time:%d", _stamp.usec(), Timestamp::now().usec());
        }
    ...
    last_batch = nullptr;
    _lock->release();
    DEBUG_CHATTER("CL:: run timer:: 19.1 p=%p last=%p\n", p, last_batch);
    int pcount = p->count();
    //p->split(pcount,last_batch);
    DEBUG_CHATTER("CL:: run timer:: 19.2 p=%p last=%p\n", p, last_batch);
    ...
    if (prev) {
        ...
        DEBUG_CHATTER("CL:: run timer:: 16.3 count=%d \n",pcount);
    }
...
Packet* Coales::simple_action(Packet *p) {
    DEBUG_CHATTER ("In CL::simple action");
     _lock->acquire();
     if (last_batch == nullptr || last_batch->count() ==0) {
