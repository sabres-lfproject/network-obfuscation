...
#define GET_END_ANNO(p,v) memcpy(&v,p->data()+END_OFFSET,1)
#define GET_START_ANNO(p,v) memcpy(&v,p->data()+START_OFFSET,1)
#define SET_END_ANNO(p) memset(p->data()+END_OFFSET,PACKET_END,1)
#define SET_START_ANNO(p) memset(p->data()+START_OFFSET,PACKET_START,1)
...
void TimedBatch::run_timer(Timer* ) {
    ...
    FOR_EACH_PACKET_SAFE(p, pkt) { // this always sends prev packet
	    ...
	    if (len > BASE_SZ) output(0).push(prev);
	    if (prev) prev->kill();
            pop_count++; // p->pop_front();
